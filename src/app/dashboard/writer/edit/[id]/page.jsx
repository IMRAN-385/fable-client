...
export default function EditEbookPage() {
  ...
  useEffect(() => {
    const fetchEbook = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/ebooks/${id}`, { headers: h });
        const data = await res.json();
        setFormData({
          title: data.ebook.title || "",
          description: data.ebook.description || "",
          fullContent: data.ebook.fullContent || "",
          price: data.ebook.price || "",
          genre: data.ebook.genre || "Fiction",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.message || "Failed to update ebook");
      }
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: "var(--ink-500)" }}>Update your ebook details</p>
      </div>
